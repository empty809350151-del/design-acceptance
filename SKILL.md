...
- Use `页面名 | Overlay Design Review and Acceptance` as the document title and show `Figma Overlay Acceptance` below the fixed `Design Acceptance` sidebar brand, and `Acceptance Issue` in the issue panel. Both the sidebar project heading and page eyebrow show the requirement name from `meta.project`, never a review status.
